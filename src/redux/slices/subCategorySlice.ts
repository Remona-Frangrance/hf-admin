...
export const addSubcategory = createAsyncThunk(
  ...
  async (subcategoryData: FormData, { rejectWithValue }) => {
    try {
      const response = await axios.post('http://localhost:5000/api/subcategories', subcategoryData, {
        headers: {
...
export const updateSubcategory = createAsyncThunk(
  ...
  async ({ id, subcategoryData }: { id: string; subcategoryData: FormData }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`http://localhost:5000/api/subcategories/${id}`, subcategoryData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
